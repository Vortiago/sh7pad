...
import { O6_CHUNK_TAG } from '../../format/chunkTags.js';
...
export function walkO6Chunks(
  ...
  let nextOff = start;
  for (const c of walkTaggedChunks(ctx.buf, start, O6_CHUNK_TAG)) {
    if (c.ver !== 0x02) {
      ...
      nextOff = c.off;
      fail(ctx, '0x06 version', `chunk @0x${c.off.toString(16)} ver=0x${c.ver.toString(16)}, expected 0x02`);
      break;
    }
    if (c.n !== classByte) {
      fail(
